			...
			"mo" => {
				...
				if !text.is_empty() && IS_WHITESPACE.is_match(text) {
					...
					self.canonicalize_mo_text(mathml);
						...
						"∪" | "∩" | "∧" | "∨" => {
							// switch to using the N-ARY versions (there other chars that could be added, but they are less common)
							let parent_name = name(get_parent(mathml));
							if matches!(parent_name, "msub" | "msubsup" | "munder" | "munderover") {
								let nary_char = match text {
									"∪" => "⋃",
									"∩" => "⋂",
									"∧" => "⋀",
									"∨" => "⋁",
	...
	fn canonicalize_mo_text(&self, mo: Element) {
		...
		if !is_base && (parent_name == "mover" || parent_name == "munder" || parent_name == "munderover") {
			...
			mo_text = match mo_text {
				...
				"◦"  |    // \u{25E6}
				"˚"       // \u{02DA}
					=> "°", // \u{00B0}, circle-like objects -> degree
				_ => mo_text,
			};
		} else {
			mo_text = match mo_text {
				...
				"˚" => "°", // \u{02DA} => \u{00B0}, raisedcircle-like objects -> degree
				_ => mo_text,
			};
		};
		if mo_text == "\u{2212}" {
